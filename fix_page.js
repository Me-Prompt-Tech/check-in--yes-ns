...
// 3. Init fetch
const init_replace_from = `        const depts = await fetchDepartmentsAction();
        setDeptOptions(depts);`;
const init_replace_to = `        const depts = await fetchDepartmentsAction();
        setDeptOptions(depts);
...
// 4. openAddModal
const open_add_from = `    setFormId('');`;
const open_add_to = `    const defaultPrefix = prefixes.length > 0 ? prefixes[0].prefix : 'EMP';
...
// 5. openEditModal
const open_edit_from = `    setFormId(emp.id);`;
const open_edit_to = `    const prefixMatch = prefixes.find(p => emp.id.startsWith(p.prefix));
...
const add_submit_from = `    if (employees.some(emp => emp.username === formUsername)) {
      setValidationError('Username นี้ถูกใช้งานแล้ว');
      return;
    }

    const newEmp: Employee = {`;
const add_submit_to = `    if (employees.some(emp => emp.username === formUsername)) {
      setValidationError('Username นี้ถูกใช้งานแล้ว');
      return;
    }
...
const edit_submit_from = `    if (employees.some(emp => emp.username === formUsername && emp.id !== selectedEmployee.id)) {
      setValidationError('Username นี้ถูกใช้งานโดยพนักงานท่านอื่นแล้ว');
      return;
    }

    const updatedEmp: Employee = {`;
const edit_submit_to = `    if (employees.some(emp => emp.username === formUsername && emp.id !== selectedEmployee.id)) {
      setValidationError('Username นี้ถูกใช้งานโดยพนักงานท่านอื่นแล้ว');
      return;
    }
...
const admin_radio_from = `                    <label className="flex items-center gap-1.5 text-xs text-slate-300 cursor-pointer">
                      <input
                        type="radio"
...
const admin_radio_to = `                    <label className={\`flex items-center gap-1.5 text-xs text-slate-300 \${formEmpPrefix !== 'ADM' && formEmpPrefix !== 'EMP' ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}\`}>
                      <input
                        type="radio"
                        checked={formRoleType === 'admin'}
                        onChange={() => setFormRoleType('admin')}
...
const admin_radio_edit_from = `                  <label className="flex items-center gap-1.5 text-xs text-slate-300 cursor-pointer">
                    <input
                      type="radio"
                      checked={formRoleType === 'admin'}
                      onChange={() => setFormRoleType('admin')}
...
const admin_radio_edit_to = `                  <label className={\`flex items-center gap-1.5 text-xs text-slate-300 \${formEmpPrefix !== 'ADM' && formEmpPrefix !== 'EMP' ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}\`}>
                    <input
                      type="radio"
                      checked={formRoleType === 'admin'}
                      onChange={() => setFormRoleType('admin')}
